...
const { tokenize } = require('./nlpHelper');
...
let faqs = [];
let tours = [];
let companyInfo = {}; // { 'key_lang': { key, language, title, content } }
let sessions = {}; // { sessionId: { history: [], context: {}, updatedAt: Date } }

/**
 * Flattens nested translation JSON objects into a flat dot-notation map
 */
...
/**
 * Initializes the in-memory database by parsing locales and data files
 */
function initMemoryDb() {
  try {
    ...
    // 1. Load Locales and Flatten them
    const locales = {};
    SUPPORTED_LANGS.forEach(lang => {
      const filePath = path.join(localesDir, `${lang}.json`);
      if (fs.existsSync(filePath)) {
        const rawJson = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        locales[lang] = flattenObject(rawJson); // FLATTEN THE OBJECT
      } else {
        locales[lang] = {};
      }
    });
    ...
    faqs = tempFaqs;
    console.log(`Loaded ${faqs.length} FAQs in memory.`);
    ...
    const defaultCompanyInfo = {
      ...
      payment_methods: {
        ...
        content: {
          ...
          it: ['Bonifico bancario sul conto della nostra azienda.', 'Link di pagamento online con Carta di Credito/Debito (Visa, Mastercard).', 'Pagamenti sicuri elaborati tramite Stripe/Paypal.'],
          ar: ['التحويل البنكي إلى حساب شركتنا.', 'رابط الدفع الإلكتروني عبر بطاقات الائتمان/الخصم (فيزا، ماستركارد).', 'المدفوعات الآمنة المعالجة عبر سترايب/بايبال.']
        }
      },
      visa_services: {
      ...
      about_us: {
        ...
        content: {
          ...
          es: 'Dunas Travel es una agencia de viajes premium especializada en tours de lujo, cruceros por el Nilo y viajes culturales en Egipto, Jordania, Grecia, Turquía y Túnez. Fundada en 2010, hemos guiado a más de 95,000 viajeros. Nos dedicamos a ofrecer itinerarios personalizados, guías profesionales multilingües y transporte privado de primer nivel para garantizar una experiencia de viaje inolvidable.',
          ...
          ar: 'دوناس ترافيل هي وكالة سفر مميزة متخصصة في الرحلات الفاخرة، كروز النيل، والرحلات الثقافية في مصر، الأردن، اليونان، تركيا، وتونس. تأسست في عام 2010، وقمنا بخدمة أكثر من 95,000 مسافر. نحن ملتزمون بتقديم برامج سفر مخصصة، مرشدين سياحيين محترفين يتحدثون عدة لغات، ووسائل نقل خاصة مريحة لضمان تجربة سفر لا تُنسى.'
        }
      }
    };
    ...
    for (const key in defaultCompanyInfo) {
      ...
      SUPPORTED_LANGS.forEach(lang => {
        const itemKey = `${key}_${lang}`;
        tempCompanyInfo[itemKey] = {
          key,
          language: lang,
    ...
    console.log(`Loaded ${Object.keys(companyInfo).length} CompanyInfo items in memory.`);

    // 4. Load & Normalize Tours
    ...
    tours = tempTours;
    console.log(`Loaded ${tours.length} normalized Tours in memory.`);
...
function saveMemorySession(sessionId, sessionData) {
  sessions[sessionId] = {
    ...sessionData,
    updatedAt: new Date()
  };
...
// Exported search capabilities
function getMemoryTours(language, filters = {}) {
  let list = tours.filter(t => t.language === language);
  ...
  if (queryDestinations.length > 0) {
    list = list.filter(t => queryDestinations.includes(t.destination));
  }
  if (filters.maxPrice) {
...
function getMemoryFaqs(language) {
  return faqs.filter(f => f.language === language);
}

function getMemoryCompanyInfo(key, language) {
  const itemKey = `${key}_${language}`;
  return companyInfo[itemKey] || null;
}
...
setInterval(() => {
  ...
  Object.keys(sessions).forEach(sid => {
    if (now - sessions[sid].updatedAt > 3600 * 1000) { // 1 hour inactivity
      delete sessions[sid];
    }
  });
}, 600 * 1000);
